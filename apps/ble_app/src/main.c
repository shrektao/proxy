...
#define BLE_SENSOR_PRIO (8)
#define BLE_SENSOR_STACK_SIZE    OS_STACK_ALIGN(512)
static struct os_task recording_task;
static volatile int g_recording_task_loops;
...
int16_t temp_record[LATEST_TEMP_COUNT];

/* 
 * reverse the temp record array to let output the latest temperature first
 */
extern void
get_temp_record(int16_t *record_array)
{
    if(record_array!=NULL) {
        for(int i=0;i<LATEST_TEMP_COUNT;i++) {
            record_array[i]=temp_record[LATEST_TEMP_COUNT-1-i];
        }
    }
}

/*
 *  The thread for recording the latest 10 temperature reading
 */
static void
...
main(void)
{
    ...
    /* Init temp_record array */
    for(int i=0;i<LATEST_TEMP_COUNT;i++) {
	temp_record[i]=0;
    }

    /* Init the thread to record the temperature history */
    pstack = malloc(sizeof(os_stack_t) * BLE_SENSOR_STACK_SIZE);
    assert(pstack);

    os_task_init(&recording_task, "ble_temp_sensor_record", ble_sensor_handler, NULL, BLE_SENSOR_PRIO, OS_WAIT_FOREVER, pstack, BLE_SENSOR_STACK_SIZE);
